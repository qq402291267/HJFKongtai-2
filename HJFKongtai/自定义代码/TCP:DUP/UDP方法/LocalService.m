...
@property (nonatomic,strong) OperatorResult * FindDeviceResult;
...
- (void)SearchDeviceWithdeviceinfo:(DevicePreF *)deviceinfo{
    ...
    [self sendToDeviceWithData:[ProtocolData discorverdevice:NewIndex deviceinfo:deviceinfo] deviceinfo:deviceinfo complete:^(OperatorResult *resultData) {
          ...
          HJFUDPLog(@"搜索发现设备结果 = %@",_FindDeviceResult);
        
        if (!_FindDeviceResult) {
...
- (void)sendToDeviceWithData:(NSData *)data deviceinfo:(DevicePreF *)deviceinfo complete:(Complete)delegate{
    ...
    HJFUDPLog(@"sendToDeviceWithData:index = %d,data = %@,lanIP = %@", currentOperator.currentIndex, data,deviceinfo.lanIP);
    ...
    if (deviceinfo && deviceinfo.lanIP) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            HJFUDPLog(@"sendboardcastAddress = %@",sendboardcastAddress);
...
//        HJFUDPLog(@">>>>----接收到无效数据:flag = 0x%x,cmd = 0x%x", flag, cmd);
        return;
    }
    HJFUDPLog(@"接收到UDP数据data = %@",data);
...
- (void)dealWithReceiveAllData:(NSData *)response
{
    ...
    if (cmd ==findDeviceOrder &&(_FindDeviceResult == nil)) {
...
- (void)UDPinterval:(NSData *)data{
    HJFUDPLog(@"%@",[NSString stringWithFormat:@"开始执行udp心跳，isMainThread = %d,isMultiThreaded = %d",[NSThread isMainThread],[NSThread isMultiThreaded]]);
    NSData * macdata = data;
...
- (void)startsendheartbeatTimer:(DevicePreF *)deviceinfo{
    HJFUDPLog(@"接收到udp心跳数据:udpinterval = %d",deviceinfo.udpinterval);
    [deviceinfo.heartbeatTimer invalidate];
    deviceinfo.heartbeatTimer = nil;
