...
{
    HJFTCPLog(@"%@",[NSString stringWithFormat:@"分析前外网数据:response = %@",response]);
    if (response == nil) {
        return nil;
    }
    //得到cmd data命令
...
//  获取工作服务器信息 0x81
//0102ffff ffffffff 0e000001 a1d26a12 81  36 94 ea 59 73 5d
...
- (NSDictionary *)getsubscribetoeventsInfo:(NSData *)response
{
    @try {
        //
        HJFUDPLog(@">>>>>>收到订阅返回数据:response = %@",response);
        UInt8 cmd = ((UInt8 *)[response bytes])[16];
...
- (NSDictionary *)getIsonlineInfo:(NSData *)response
{
    @try {
        ...
        if (deviceinfo) {
            ...
            HJFTCPLog(@">>>>>>>>>>>>>>>>>>>>>TCP查询设备是否在线,remoteIsOnline = %d,localIsOnline = %d",deviceinfo.remoteIsOnline,deviceinfo.localIsOnline);
...
- (NSDictionary *)setdeviceIOstatusInfo:(NSData *)response
{
    @try {
        ...
        UInt8 result = ((UInt8 *)[response bytes])[19];//00
//        BOOL isSuccess = (result !=deviceInfo.deviceOn);
        NSData * macdata = [self getProtcolmac:response];
        DevicePreF *deviceInfo = [DeviceManageInstance getDevicePreFWithmac:macdata];
        BOOL isSuccess = (result !=deviceInfo.deviceOn);
        deviceInfo.deviceOn =(result ==0xFF);
...
- (NSDictionary *)getgetIOstatusInfo:(NSData *)response
{
    ...
    @try {
        ...
        UInt8 cmd = ((UInt8 *)[response bytes])[16];
        UInt8 Control_Status = ((UInt8 *)[response bytes])[19];
...
- (UInt8)getProtcolCmd:(NSData *)response
{
    if (response.length < 17) {
        HJFTCPLog(@"getProtcolCmd:接收到错误数据");
        return 0;
    }
    UInt8 cmd = ((UInt8 *)[response bytes])[16];
    return cmd;
...
- (UInt16)indexFromResponse:(NSData *)response
{
    ...
    HJFTCPLog(@"%@",[NSString stringWithFormat:@"----------index------------ = %d",index]);
    return index;
